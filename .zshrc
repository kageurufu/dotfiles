...
aurg() {
  package_name=$1
  package-query -A "$package_name"

  if [ $? != 0 ]; then
    package-query -As "$package_name"
    if [ $? != 0 ]; then
      echo "No packages found"
      return 1
    fi
    echo "Please select your package with \"aurg [package-name]\""
    return 0
  fi
  filename=${package_name}.tar.gz
  url=https://aur.archlinux.org/packages/${package_name:0:2}/${package_name}/${filename}
  wget ${url}
  if [ $? != 0 ]; then
